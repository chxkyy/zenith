...
import { Table, Button, Tag, Modal, Form, Input, Select, Space, App, Switch, Card } from 'antd';
...
export default function ProcessTemplateTable() {
  ...
  // Local search params for name + status filter form binding
  ...
  const {
    ...
    refresh,
    refetchWithQuery,
  } = usePaginatedQuery<ProcessTemplate>({
    apiUrl: '/api/workflow/process-templates/page',
    initialPageSize: 20,
  });
  ...
  const {
    ...
    openEditModal,
    closeModal,
  } = useCrudModal<ProcessTemplate>({
    onOpenAdd: () => {
      form.resetFields();
      setNodes([]);
    },
    onOpenEdit: async (record) => {
      try {
        const detail = await get<any>('/api/workflow/process-templates/detail', { id: record.id });
        form.setFieldsValue({
          id: detail.id,
          code: detail.code,
          name: detail.name,
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      setNotification({
        message:
          modalMode === 'edit' ? '流程模板更新成功' : '流程模板创建成功',
        type: 'success',
        key: Date.now(),
      });
    } catch (err) {
      const msg =
        err instanceof Error && err.message
          ? err.message
          : '保存失败，请重试';
      setNotification({ message: msg, type: 'error', key: Date.now() });
    }
  };
  ...
  const handleUpdateStatus = async (id: number, status: number) => {
    try {
      await post('/api/workflow/process-templates/update-status', { id, status });
      refresh();
      setNotification({
        message: status === 1 ? '流程模板已启用' : '流程模板已停用',
        type: 'success',
        key: Date.now(),
      });
    } catch (err) {
  ...
  const handleNodeChange = (index: number, field: string, value: unknown) => {
    ...
    if (field === 'approverType') {
      newNodes[index].approverTypeName =
        value === 1 ? '角色' : value === 2 ? '用户' : '发起人上级';
    }
    setNodes(newNodes);
  };
                  ...
                  >
                    ...
                    >
                      <Select
                        value={node.nodeType}
                        onChange={(val) =>
                          handleNodeChange(index, 'nodeType', val)
                        }
                        options={[
                          { value: 1, label: '普通审批' },
                          { value: 2, label: '会签' },
                    ...
                    >
                      <Select
                        ...
                        onChange={(val) =>
                          handleNodeChange(index, 'approverType', val)
                        }
                        options={[
                          { value: 1, label: '角色' },
                          { value: 2, label: '用户' },
